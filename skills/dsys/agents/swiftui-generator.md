...
- `component_manifest_path`: (optional) Path to component-manifest.json. If provided and valid, generate additional components beyond the base 6. If absent or invalid, generate only the base 6.
...
## Step 1b: Load Component Manifest (optional)

If `component_manifest_path` is provided:

1. Use **Read** to load the file. If Read fails: set `detected_components = []` and continue.
2. Parse the JSON. If parse fails: set `detected_components = []` and continue.
3. Extract the `detected_components` array.
4. For each entry, validate the `name` does not collide with: `Button`, `Card`, `Input`, `Badge`, `Heading`, `Text`. If a collision is found, skip that entry.
5. Set `detected_components` to the validated list.

If `component_manifest_path` is not provided: set `detected_components = []`.
...
## Step 11b: Write Detected Component Files

**Skip this step if `detected_components` is empty.**

For each entry in `detected_components`, write `{output_root}/Components/DS{name}.swift`.

Follow these rules for every detected component:

**Rules (same as base components — non-negotiable):**
- All struct names use the `DS` prefix (e.g., `DSStatTile`, `DSIconCircle`)
- Use ONLY design system tokens: `Color.dsX`, `DSFont.X()`, `DSRadius.X`, `.dsShadow(.X)`, `DSSpacingFixed.X`
- NEVER use `Color.blue`, `Color.red`, `Font.body`, `Font.title`, `UIColor`, or raw hex literals
- All structs are `public struct DS{Name}: View`
- Include a `#Preview` block (Xcode 15 style)

**Generating the component body:**

Use the manifest entry's fields to guide generation:

1. **Struct properties** — Build from `props_hint` entries. Add size/variant enums from `visual_properties.size_variants` and `visual_properties.color_variants` if present.

2. **Token mapping** — Map `token_usage` to SwiftUI design system tokens:
   - `token_usage.colors`: `"action-primary"` → `Color.dsActionPrimary`, `"surface-raised"` → `Color.dsSurfaceRaised`, `"text-muted"` → `Color.dsTextMuted`, `"border-default"` → `Color.dsBorderDefault`
   - `token_usage.radius`: `"sm"` → `DSRadius.sm`, `"md"` → `DSRadius.md`, `"lg"` → `DSRadius.lg`, `"full"` → `DSRadius.full`
   - `token_usage.shadow`: `"sm"` → `.dsShadow(.sm)`, `"md"` → `.dsShadow(.md)`
   - `token_usage.spacing`: Map to `DSSpacingFixed` values (e.g., `p-4` → `.padding(DSSpacingFixed.md)`)
   - `token_usage.typography`: Map to `DSFont` functions (e.g., `text-sm` → `DSFont.caption()`, `font-bold` → `.fontWeight(.bold)`)

3. **Structure by complexity:**
   - `simple`: Single element view. Example:
     ```swift
     public var body: some View {
         Image(systemName: iconName)
             .foregroundColor(Color.dsTextPrimary)
             .frame(width: size.dimension, height: size.dimension)
             .background(Color.dsSurfaceOverlay)
             .clipShape(Circle())
     }
     ```
   - `compound`: 2-3 child views composed with HStack/VStack:
     ```swift
     public var body: some View {
         HStack(spacing: DSSpacingFixed.sm) {
             icon
             VStack(alignment: .leading, spacing: DSSpacingFixed.xs) {
                 Text(title).font(DSFont.body()).foregroundColor(Color.dsTextPrimary)
                 Text(subtitle).font(DSFont.caption()).foregroundColor(Color.dsTextMuted)
             }
         }
         .padding(DSSpacingFixed.md)
         .background(Color.dsSurfaceRaised)
         .cornerRadius(DSRadius.lg)
     }
     ```
   - `complex`: Multiple child views with variants and conditional rendering.

4. **Variant handling** — If `visual_properties.color_variants` or `visual_properties.size_variants` exist, create Swift enums:
   ```swift
   public enum Variant: String, CaseIterable {
       case `default`, active, highlighted
   }
...
**Example detected component output:**

```swift
// DSStatTile.swift
// Generated by dsys (detected component) — do not edit manually
...
public struct DSStatTile: View {
    public enum Size: CaseIterable {
        ...
        var padding: CGFloat {
            switch self {
            case .sm: DSSpacingFixed.sm
            case .md: DSSpacingFixed.md
            }
        }
    }
    ...
    public var body: some View {
        HStack(spacing: DSSpacingFixed.sm) {
            ...
            VStack(alignment: .leading, spacing: DSSpacingFixed.xs) {
                Text(value)
                    .font(DSFont.heading3())
                    .foregroundColor(Color.dsTextPrimary)
                Text(label)
                    .font(DSFont.caption())
                    .foregroundColor(Color.dsTextMuted)
            }
            if let trend {
                Spacer()
                trend
...
#Preview("DSStatTile") {
    VStack(spacing: 16) {
        DSStatTile(value: "1,234", label: "Total Users", size: .md)
        DSStatTile(value: "56%", label: "Conversion", size: .sm)
    }
    .padding()
    .background(Color.dsSurfaceDefault)
}
...
{if detected_components is non-empty:}
// Detected components (from screenshot analysis):
// {for each: DS{Name1}, DS{Name2}, ...}
// See component-manifest.json for detection details.
{end if}
```
...
- [ ] All 6 base component files include a `#Preview` block with the Xcode 15 comment
...
**Detected components (if any):**
- [ ] All detected component `.swift` files exist in `Components/` with `DS` prefix
- [ ] All detected components use ONLY design system tokens (no `Color.blue`, `Font.body`, raw hex)
- [ ] All detected components are `public struct` conforming to `View`
- [ ] All detected components include a `#Preview` block
...
```
Generated SwiftUI design system: {file_count} files in {output_root}{if detected_components is non-empty:  ({detected_count} detected components)}
```

Where `{file_count}` is the total number of files written (Swift files + JSON files, including detected component files).
