...
## Step 5b: Build Extension Rules Section

Generate a section that guides future AI sessions (and human developers) on how to extend the design system without breaking the regeneration flow.
...
When building new components or extending the design system, follow these rules to maintain consistency and avoid breaking regeneration.

**New components MUST use existing semantic tokens.**
- NEVER introduce new hex color values in component code. All colors come from the semantic token palette.
  Does this new component contain a hex color (`#xxxxxx`) or a non-token color reference? VIOLATION.
- NEVER create ad-hoc spacing or radius values. Use the existing scale.
  Does this new component use a spacing or radius value not from the design system scale? VIOLATION.

**New components should follow established prop API patterns.**
```

After the opening paragraphs above, emit platform-conditional sub-sections:

**React sub-section (emit only if `"react"` in `platforms`):**

```markdown
- React components: accept `variant`, `size`, and `className` props matching existing component conventions (Button, Card, Input, Badge, Heading, Text).
```

**SwiftUI sub-section (emit only if `"swiftui"` in `platforms`):**

```markdown
- SwiftUI components: use a `DS` prefix and accept `variant` and `size` parameters matching existing DS-prefixed component conventions (DSButton, DSCard, DSInput, DSBadge, DSHeading, DSText).
```

**Continue with platform-conditional custom directory rules:**

```markdown
**Custom components live in a separate directory — generated files are overwritten on re-run.**
```

**React directory rule (emit only if `"react"` in `platforms`):**

```markdown
- Place custom React components in a `custom/` directory alongside the generated components (e.g., `components/custom/Modal.tsx`). NEVER modify files inside the generated `components/` directory — they will be overwritten.
  Does this change modify a file that contains a "Generated by dsys" header comment? VIOLATION.
```

**SwiftUI directory rule (emit only if `"swiftui"` in `platforms`):**

```markdown
- Place custom SwiftUI components in a `Custom/` directory alongside the generated Components (e.g., `Components/Custom/DSModal.swift`). NEVER modify files inside the generated `Components/` directory.
  Does this change modify a file that contains a "Generated by dsys" header comment? VIOLATION.
```

**Continue with the source-of-truth and escape-hatch rules (always emit):**

```markdown
**Adding new semantic tokens: modify the source, not the output.**
- If you need a new semantic token (e.g., a new feedback color), add it to `design-system.json` and re-run dsys. NEVER hardcode token values in component code.
- `design-system.json` is the single source of truth. Generated token files are derived outputs.

**Escape hatch for intentional one-off overrides.**
```

**React escape hatch (emit only if `"react"` in `platforms`):**

```markdown
- For an intentional one-off rule exception, add a `/* dsys-override — {reason} */` comment on the same line.
  Example: `className="bg-[#FF0000]" /* dsys-override — temporary brand campaign color */`
```

**SwiftUI escape hatch (emit only if `"swiftui"` in `platforms`):**

```markdown
- For an intentional one-off rule exception, add a `// dsys-override — {reason}` comment on the same line.
  Example: `.foregroundStyle(Color(hex: "#FF0000")) // dsys-override — temporary brand campaign color`
```

**Always emit:**

```markdown
- Every `dsys-override` comment MUST include a reason. A bare `dsys-override` without explanation is a VIOLATION.
```
...
### 8b. File Ownership & Extension Points

This section helps users understand which files are safe to modify and where to extend the system. Emit it directly after the header and vibe narrative.

**Path derivation:** Construct platform output paths from the `output_dir` parameter:
- `react_output_root` = `{output_dir}react/src/design-system/`
- `swiftui_output_root` = `{output_dir}swiftui/Sources/DesignSystem/`
...
> **Rule:** Files generated by dsys will be overwritten on re-run. Files you create outside generated directories are always safe.

### Generated Files (do not edit — overwritten by dsys)
```

**Tokens & Config sub-section — conditional on platforms:**

If `"react"` in `platforms`, emit:
```markdown
**Tokens & Config (React/Tailwind)**
- `{react_output_root}tokens/tokens.css` — CSS custom properties (light + dark)
- `{react_output_root}tokens/theme.css` — Tailwind v4 @theme configuration
- `{react_output_root}tokens/tokens.json` — W3C DTCG source tokens
- `{react_output_root}types/design-tokens.d.ts` — TypeScript token type unions
- `{react_output_root}index.ts` — barrel export
```

If `"swiftui"` in `platforms`, emit:
```markdown
**Tokens & Config (SwiftUI)**
- `{swiftui_output_root}Colors.xcassets/` — asset catalog with color sets
- `{swiftui_output_root}Colors+DesignSystem.swift` — Color extensions
- `{swiftui_output_root}Typography+DesignSystem.swift` — DSFont struct
- `{swiftui_output_root}Spacing+DesignSystem.swift` — DSSpacing struct
- `{swiftui_output_root}Radius+DesignSystem.swift` — DSRadius constants
- `{swiftui_output_root}Shadows+DesignSystem.swift` — DSShadow modifier
- `{swiftui_output_root}DesignSystem.swift` — barrel re-export
```

**Components sub-section — conditional on platforms:**

If `"react"` in `platforms`, emit:
```markdown
**Components (React)**
- `{react_output_root}components/Button.tsx`
- `{react_output_root}components/Card.tsx`
- `{react_output_root}components/Input.tsx`
- `{react_output_root}components/Badge.tsx`
- `{react_output_root}components/Heading.tsx`
- `{react_output_root}components/Text.tsx`
```

If `"swiftui"` in `platforms`, emit:
```markdown
**Components (SwiftUI)**
- `{swiftui_output_root}Components/DSButton.swift`
- `{swiftui_output_root}Components/DSCard.swift`
- `{swiftui_output_root}Components/DSInput.swift`
- `{swiftui_output_root}Components/DSBadge.swift`
- `{swiftui_output_root}Components/DSHeading.swift`
- `{swiftui_output_root}Components/DSText.swift`
```

**Documentation sub-section (always emit):**

```markdown
**Documentation**
- `CLAUDE.md` — only the `<!-- dsys:rules:start -->` ... `<!-- dsys:rules:end -->` block (rest of the file is yours)
- `STYLE-GUIDE.md` — this file
- `preview.html` — visual preview
```

**User-owned extension points sub-section:**

```markdown
### User-Owned Extension Points (safe to create and modify)
```

If `"react"` in `platforms`, emit:
```markdown
- `{react_output_root}components/custom/` — place your custom React components here
- Any file outside `{react_output_root}` that imports from the design system
```

If `"swiftui"` in `platforms`, emit:
```markdown
- `{swiftui_output_root}Components/Custom/` — place your custom SwiftUI components here
- Any file outside `{swiftui_output_root}` that imports `DesignSystem`
```

**Always emit:**
```markdown
- `{output_dir}design-system.json` — add new tokens here, then re-run dsys to regenerate code

### How CLAUDE.md Rules Work

The dsys rules block in your project's CLAUDE.md is wrapped in section markers:

<!-- dsys:rules:start — generated by dsys on YYYY-MM-DD, do not edit manually -->
...rules...
<!-- dsys:rules:end -->

Only content between these markers is managed by dsys. Everything else in your CLAUDE.md is untouched on re-run. You can safely add your own project rules, team conventions, or overrides outside the markers.
```
...
- [ ] Extension Rules section is present after Token Rules
- [ ] Extension Rules contains at least 2 instances of "VIOLATION" test pattern
- [ ] Extension Rules mentions the `dsys-override` escape hatch comment syntax
- [ ] Extension Rules references a `custom/` directory for new components
...
- [ ] File Ownership section is present after the header/vibe section
- [ ] File Ownership lists generated files grouped by category (tokens/config, components, documentation)
- [ ] File Ownership lists user-owned extension points including `custom/` component directories
- [ ] File Ownership explains the `dsys:rules:start`/`dsys:rules:end` marker system
- [ ] File Ownership paths use actual output directory values, not raw placeholders
