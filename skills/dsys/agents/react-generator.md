...
## Step 7b: Write Detected Component Files

**Skip this step if `detected_components` is empty.**

For each entry in `detected_components`, write `{output_root}/components/{name}.tsx`.
...
- Accept `className` prop, merge as trailing: `${className ?? ""}`
- Use ONLY semantic Tailwind class names: `bg-primary`, `text-text-muted`, `border-border`, etc.
- NEVER use raw hex values, Tailwind default colors, or arbitrary values
- Set `displayName` after the component declaration
- Export the component as `export default ComponentName`
- Export the props interface as `export interface ComponentNameProps`
- Include file header comment: `// {Name}.tsx — generated by dsys (detected component)`

**Generating the component body:**

Use the manifest entry's fields to guide generation:

1. **Props interface** — Build from `props_hint` entries. Add size/variant unions from `visual_properties.size_variants` and `visual_properties.color_variants` if present. Always include `className?: string` and `children?: React.ReactNode` (for compound/complex components).

2. **Token mapping** — Map `token_usage` to Tailwind classes:
   - `token_usage.colors`: `"action-primary"` → `bg-primary`, `"surface-raised"` → `bg-surface-raised`, `"text-muted"` → `text-text-muted`, `"border-default"` → `border-border`
   - `token_usage.radius`: `"sm"` → `rounded-sm`, `"md"` → `rounded-md`, `"lg"` → `rounded-lg`, `"full"` → `rounded-full`
   - `token_usage.shadow`: `"sm"` → `shadow-sm`, `"md"` → `shadow-md`
   - `token_usage.spacing`: Use directly as Tailwind classes (`p-4`, `gap-2`, etc.)
   - `token_usage.typography`: Use directly as Tailwind classes (`text-sm`, `font-bold`, etc.)

3. **Structure by complexity:**
   - `simple`: Single element wrapper. One `<div>` or `<span>` with the token classes applied.
   - `compound`: 2-3 child areas. Use a flex container with child slots. Example:
     ```tsx
     ...
     ```
   - `complex`: Multiple child areas with variants. Use a container with named slots and variant classes.

4. **Variant handling** — If `visual_properties.color_variants` or `visual_properties.size_variants` exist, create variant/size maps (same pattern as Button's `variants` and `sizes` objects).

**Example detected component output:**

```tsx
// StatTile.tsx — generated by dsys (detected component)
// Do not edit manually — re-run dsys to regenerate
...
const StatTile = forwardRef<HTMLDivElement, StatTileProps>(
  ({ icon, value, label, trend, size = "md", className, ...props }, ref) => {
    const sizes = {
      sm: "p-3 gap-2",
      md: "p-4 gap-3",
    };
    return (
      <div
        ref={ref}
        className={`flex items-center bg-surface-raised rounded-lg border border-border shadow-sm ${sizes[size]} ${className ?? ""}`}
        {...props}
      >
        {icon && (
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-overlay text-text-secondary">
            {icon}
          </span>
        )}
        <div className="flex flex-col">
...
// Detected Components (from component manifest)
{for each entry in detected_components:}
...
If `detected_components` is empty, omit the "Detected Components" comment block entirely.
...
- [ ] All 6 base components have `forwardRef`, `displayName`, and `className` prop
- [ ] `index.ts` exports all 6 base components and their Props types

**Detected components (if any):**
- [ ] All detected component `.tsx` files exist in `components/`
- [ ] All detected components use ONLY semantic Tailwind classes (no raw hex, no default colors)
- [ ] All detected components have `forwardRef`, `displayName`, and `className` prop
- [ ] `index.ts` exports all detected components and their Props types

---

## Step 11: Return Summary

After all files are written and verified, return exactly this line:

```
Generated React/Tailwind design system: {file_count} files in {output_root}{if detected_components is non-empty:  ({detected_count} detected components)}
```

Where `{file_count}` is the count of files successfully written (base target: 11 non-.bak files, plus one additional `.tsx` file per detected component).
