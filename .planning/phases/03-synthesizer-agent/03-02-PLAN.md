...
must_haves:
  truths:
    ...
    - "Feedback colors that were null in the single finding are derived with sensible defaults and documented via $description (not conflict_log, since derivations are not multi-source conflicts)"
...
3. **Semantic Color Merge**: With N=1, there are no conflicts to resolve. Each non-null semantic assignment maps directly to the output. For null assignments (action_secondary, action_secondary_dark, action_destructive, action_destructive_dark, feedback_success, feedback_warning, feedback_info), derive sensible defaults:
   ...
   Document each derived value in the token's `$description` field (e.g., `$description: "Derived: standard red — no benchmark contained this role"`). Do NOT add derived tokens to conflict_log — they are not multi-source conflicts and would fail schema validation (candidates requires minItems:2).
...
**Then validate** (npx ajv-cli is the established validation pattern from Phase 2 — see 02-02-SUMMARY.md; it downloads on-demand via npx if not cached):
```bash
...
- conflict_log candidates minItems:2 → derived tokens must NOT be in conflict_log (they use $description instead); conflict_log should be empty [] for N=1 source with no multi-source conflicts
...
- conflict_log should be empty [] (N=1 source, no multi-source conflicts). Derived tokens (null feedback colors, derived overlay/inset/secondary/link) should be documented via their $description fields instead.
  </action>
  <verify>
...
- `python3 -c "import json; d=json.load(open('.dsys/design-system.json')); print('font:', d['tokens']['typography']['font_family']['sans']['$value'])"` prints `Satoshi`
  ...
  <done>
`.dsys/design-system.json` exists, passes ajv-cli schema validation with --spec=draft2020, contains all 18 semantic color roles populated (including derived overlay/inset/secondary/link), has empty conflict_log [] (no multi-source conflicts for N=1), derived tokens documented via $description fields, reflects the Luxora aesthetic (bold tone, comfortable density, Satoshi font, forest-green palette), and is human-inspectable on disk.
  </done>
</task>
...
5. conflict_log is empty [] (N=1 source, no multi-source conflicts). Derived tokens (feedback_success, feedback_warning, feedback_info, action_secondary, action_destructive, surface.overlay, surface.inset, text.secondary, text.link) are documented via their $description fields with "Derived:" prefix.
...
- Derived tokens (not directly observed) are documented via their $description fields; conflict_log is reserved for multi-source conflicts only (empty [] for N=1 source)
