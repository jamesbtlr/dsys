...
**Check for component manifest before issuing generator Tasks:**
```bash
...
platforms: [\"react\"]
{if HAS_MANIFEST: }component_manifest_path: .dsys/{name}/component-manifest.json{end if}"
)
```
...
Components: Button, Card, Input, Badge, Heading, Text{if component-manifest.json exists and has detected_components: , {DetectedName1}, {DetectedName2}, ...}
