...
4. Add a THIRD `allOf` condition to relax the `ui_screenshot` constraint when `partial_failure` is true. The existing second `allOf` entry forces typography and spacing to be objects for `ui_screenshot`, but when extraction partially fails, these fields may need to be null. Add this condition:
```json
{
...
```
This third `allOf` entry must appear AFTER the existing two. When `partial_failure` is NOT present or is false, the second `allOf` entry (requiring objects for `ui_screenshot`) still applies. When `partial_failure` IS true, both the second and third entries match, but JSON Schema `allOf` requires ALL conditions to pass — the third entry's `then` widens the type to `["object", "null"]`, which is compatible with the second entry's `then` of `{ "type": "object" }` only when the if-condition of the THIRD entry matches (because `allOf` applies each if/then independently). NOTE: This works because in JSON Schema, when an `if` matches, the `then` applies; when the `if` does NOT match, the `then` is ignored. So for a `ui_screenshot` WITHOUT `partial_failure: true`, only the second `allOf` fires (requiring object). For a `ui_screenshot` WITH `partial_failure: true`, the third `allOf` also fires and permits null.

IMPORTANT CONTEXT COMPLIANCE NOTE: The locked decision in CONTEXT.md states "Omit missing categories from output JSON entirely." In the schema, `typography` and `spacing` are in the `required` array and cannot be omitted from the JSON object without breaking schema validation. The interpretation implemented here is: when extraction partially fails, set the failed category to `null` (the schema-conformant way to signal absence) and record which categories failed in `failed_categories`. The `partial_failure` + `failed_categories` fields are the mechanism that communicates "these categories are missing" — the consumer can treat null + failed_categories equivalently to omission. This is the only schema-conformant approach since `required` fields cannot be conditionally omitted in JSON Schema draft 2020-12 without removing them from `required` entirely (which would break the normal case).
...
5. For `ui_screenshot` WITHOUT `partial_failure: true`: typography and spacing are objects (not null)
6. For `ui_screenshot` WITH `partial_failure: true`: skip the object-type check for fields listed in `failed_categories` (they may be null)
7. All hex values match the pattern `#` followed by exactly 6 hex characters
...
Light/dark mode pair awareness (locked decision from CONTEXT.md):
- In Phase 2, each analyzer agent instance processes ONE image independently and reports its `background_style` (light/dark/unknown) in the output
- The agent does NOT combine findings across images — it has no knowledge of other images being analyzed in parallel
- Pair recognition and combined findings happen in Phase 3 (Synthesizer), which receives all findings and can detect when two images are the same UI in different modes based on matching structure + opposite background_style values
- This is an explicit architectural note: the per-image agent architecture means "pair awareness" = each agent faithfully reports its theme, and the synthesizer does the pairing
